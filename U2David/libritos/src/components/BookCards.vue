...
export default {
  ...
  data() {
    return {
      ...
      endDate: null,
      startDate: null,
       lastScrollPosition: 1, // Valor inicial de la última posición de desplazamiento
    scrollDistance: 1, // Valor inicial de la distancia de desplazamiento
  ...
  methods: {
    ...
    ordenarLibrosDesc() {
      this.loading = true;
      axios
        .get("http://localhost:8080/books/autorDesc")
        .then((response) => {
          this.books = response.data;
          this.loading = false;
        })
    ...
    ordenarLibrosAutor() {
      this.loading = true;
      axios
        .get("http://localhost:8080/books/autorAsc")
        .then((response) => {
          this.books = response.data;
          this.loading = false;
        })
